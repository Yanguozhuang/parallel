...
namespace
{
    ...
    // one_block_step 在后面定义，这里先声明，方便线程函数调用。
    static void one_block_step(Matrix &U, Matrix &B, Matrix &V, int l, int r);

    // ===== pthread 简单线程池版本参数 =====
    // 想改线程数：编译时加 -DSVD_THREAD_NUM=2/4/8
#ifndef SVD_THREAD_NUM
#define SVD_THREAD_NUM 4
#endif
    ...
    struct Pool
    {
        ...
        std::vector<long long> total_tasks;
        std::vector<long long> total_work;

        Pool() : U(nullptr), B(nullptr), V(nullptr), next_task(0), active_workers(0), has_work(false), stop(false)
        {
            pthread_mutex_init(&mutex, nullptr);
            pthread_cond_init(&cond_start, nullptr);
            pthread_cond_init(&cond_done, nullptr);

            threads.resize(SVD_THREAD_NUM);
            args.resize(SVD_THREAD_NUM);
            total_tasks.assign(SVD_THREAD_NUM, 0);
            total_work.assign(SVD_THREAD_NUM, 0);

            for (int t = 0; t < SVD_THREAD_NUM; ++t)
            {
                args[t].pool = this;
                args[t].tid = t;
                pthread_create(&threads[t], nullptr, worker_entry, &args[t]);
            }
        }
        ...
        {
            PoolArg *arg = static_cast<PoolArg *>(p);
            arg->pool->worker(arg->tid);
            return nullptr;
        }
        ...
        void worker(int tid)
        {
            while (true)
            {
                ...
                {
                    while (has_work && next_task >= static_cast<int>(tasks.size()) && !stop)
                    {
                        pthread_cond_wait(&cond_start, &mutex);
                    }
                    pthread_mutex_unlock(&mutex);
                ...
                pthread_mutex_lock(&mutex);
                total_tasks[tid]++;
                total_work[tid] += block_work(blk);
                active_workers--;

                if (next_task >= static_cast<int>(tasks.size()) && active_workers == 0)
                {
                    has_work = false;
        ...
        void run(Matrix &u, Matrix &b, Matrix &v, const std::vector<Block> &blocks)
        {
            std::vector<Block> new_tasks;
            for (int i = static_cast<int>(blocks.size()) - 1; i >= 0; --i)
            {
                if (blocks[i].r > blocks[i].l)
                {
                    new_tasks.push_back(blocks[i]);
                }
            }
        ...
        void print_load() const
        {
            ...
            std::fprintf(stderr, "[pool pthread load]\\n");
            for (int t = 0; t < SVD_THREAD_NUM; ++t)
            {
                std::fprintf(stderr, "thread %d: tasks=%lld, work=%lld\\n", t, total_tasks[t], total_work[t]);
            }
#endif
        }
    };
...
bool gkh_svd_from_bidiagonal(Matrix &U, Matrix &B, Matrix &V, int max_iter, double tol)
{
    ...
    for (int iter = 0; iter < max_iter; ++iter)
    {
        ...
        // 线程池 pthread：线程只创建一次，每轮动态领取活动块。
        pool.run(U, B, V, blocks);
    }
